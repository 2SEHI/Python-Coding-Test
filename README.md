# Programming Test

```
📁programmers							# 💻프로그래머스
├───📃absolute.py						# level1 음양더하기
├───📃clawCrane.py						# level1 인형뽑기(2019 카카오 인턴쉽)
├───📃compressSentence.py				# level2 문자열 압축(2020 카카오 블라인드테스트)
├───📃key_pad.py						# level1 키패드 누르기(2020 카카오 인턴쉽)
├───📃lotto.py							# level1 로또의 최고 순위와 최저 순위
├───📃minRectangle.py					# Weekly Challenge 8주차_최소 직사각형
├───📃openChat.py						# level2 오픈채팅방(2019 카카오 블라인드테스트)
├───📃recomNewID.py						# level1 신규 아이디 추천
├───📃StrToNum.py						# level1 숫자 문자열과 영단어
├───📃weekly1.py						# Weekly Challenge 1주차_부족한 금액 계산하기
├───📃weekly2.py						# Weekly Challenge 2주차_상호평가
├───📃weekly3.py						# Weekly Challenge 3주차_퍼즐조각채우기(미완성)
├───📃weekly5.py						# Weekly Challenge 5주차_모음사전
└───📃uniform.ipynb						# level1 체육복

📁python-algorithm-interview 					# 📘파이썬 알고리즘 인터뷰(💻leetcode)
├───📃2_AddTwoNumbers.py					# 8장 16번_두수의 덧셈
├───📃15_3sum.py						# 7장 09번_세 수의 합	
├───📃23_MergeKSortedLists.py					# 10장 27번_k개 정렬 리스트 병합
├───📃125_ValidPlindrome.py					# 6장 01번_유효한 팰린드롬
├───📃622_DesignCircularQueue.py				# 9장 25번_원형큐디자인
└───📃641_DesignCircularDeque.py				# 10장 26번_원형데크디자인

📁python-coding-dojang 						# 📘파이썬 코딩 도장
└───📃unit39-7_iterator.py					# unit39.7 시간 이터레이터 만들기

📁python-for-coding-test					# 📘이것이 코딩 테스트다
├───📃4-2.py							# Chapter4-2 왕실의 나이트
└───📃4-3.py							# Chapter4-3 게임 개발
```




## [programmingTest 풀이보러가기(일부만 있음)](https://2sehi.github.io/categories/#programming-test)

